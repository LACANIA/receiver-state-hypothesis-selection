"""MA-BGTR-v6 Pareto/dominance model-selection layer."""
...
from .pareto_selector import ParetoSelectorConfig, select_group
...
def _first_matching_error(group: pd.DataFrame, model: str) -> float:
...
def _selected_result(keys: tuple[Any, ...], group: pd.DataFrame, selected: pd.Series, reason: str, low_quality: bool) -> dict[str, Any]:
    ...
    ctd_error = _first_matching_error(group, "M2_ctd_full")
    static_error = _first_matching_error(group, "M0_static_position")
    return {
        "dataset_type": keys[0],
        "scenario": keys[1],
        ...
        "selected_model_v6": selected.get("candidate_model"),
        "selection_reason_v6": reason,
        ...
        "robust_validation_cost": selected.get("robust_validation_cost", np.nan),
...
def select_v6_results(candidate_df: pd.DataFrame, config: ParetoSelectorConfig | None = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Run MA-BGTR-v6 selection over grouped candidate rows."""
    cfg = config or ParetoSelectorConfig()
    per_candidate: list[pd.DataFrame] = []
    frontier_rows: list[pd.DataFrame] = []
    dominance_rows: list[pd.DataFrame] = []
    selected_rows: list[dict[str, Any]] = []

    for keys, group in candidate_df.groupby(GROUP_KEYS, dropna=False, sort=True):
        annotated, frontier, dominance, selected, reason, low_quality = select_group(group, cfg)
        for frame in (annotated, frontier, dominance):
            for key, value in zip(GROUP_KEYS, keys):
                if key not in frame.columns:
                    frame[key] = value
        per_candidate.append(annotated)
    ...
    candidates = pd.concat(per_candidate, ignore_index=True) if per_candidate else pd.DataFrame()
    frontier_df = pd.concat(frontier_rows, ignore_index=True) if frontier_rows else pd.DataFrame()
    dominance_df = pd.concat(dominance_rows, ignore_index=True) if dominance_rows else pd.DataFrame()
    selected_df = pd.DataFrame(selected_rows)
    return candidates, selected_df, frontier_df, dominance_df
...
def aggregate_v6_selected(selected_df: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for keys, group in selected_df.groupby(["dataset_type", "scenario"], sort=True):
        counts = group["selected_model_v6"].value_counts()
        ...
        rows.append(
            {
                ...
                "most_selected_model_v6": str(counts.index[0]) if len(counts) else "",
                "selected_model_entropy_v6": entropy(counts),
...
def model_selection_counts_v6(selected_df: pd.DataFrame) -> pd.DataFrame:
    counts = selected_df.groupby(["dataset_type", "scenario", "selected_model_v6"]).size().reset_index(name="count")
    totals = counts.groupby(["dataset_type", "scenario"])["count"].transform("sum")
    counts["fraction"] = counts["count"] / totals
    return counts


def compare_v5_v6(v5_selected: pd.DataFrame, v6_selected: pd.DataFrame) -> pd.DataFrame:
    keys = GROUP_KEYS
    if v5_selected.empty or v6_selected.empty:
        return pd.DataFrame()
    merged = v5_selected.merge(v6_selected, on=keys, how="inner", suffixes=("_v5", "_v6"))
    return pd.DataFrame(
        {
            ...
            "selected_model_v5": merged["selected_model_v5"],
            "selected_model_v6": merged["selected_model_v6"],
            "v5_final_position_error_m": merged["final_position_error_m_v5"],
            "v6_final_position_error_m": merged["final_position_error_m_v6"],
            "improvement_m": merged["final_position_error_m_v5"] - merged["final_position_error_m_v6"],
            "v5_trimmed_validation_rmse_mps": merged["trimmed_validation_rmse_mps_v5"],
            "v6_trimmed_validation_rmse_mps": merged["trimmed_validation_rmse_mps_v6"],
            "v6_selection_reason": merged["selection_reason_v6"],
        }
    )
