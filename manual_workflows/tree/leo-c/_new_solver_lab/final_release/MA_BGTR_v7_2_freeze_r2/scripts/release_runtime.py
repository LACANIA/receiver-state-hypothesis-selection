...
def initial_guess_for_labels(
    obs: dict[str, Any], position_init_label: str, velocity_init_label: str, beta_prior_profile: str
) -> dict[str, Any]:
    pos_label = position_init_label
    vel_label = velocity_init_label
    east, _north, _up = enu_axes(float(obs["lat_deg"]), float(obs["lon_deg"]))
    p_true = np.asarray(obs["p0_true_m"], dtype=float)
    v_true = np.asarray(obs["v_true_mps"], dtype=float)
    if pos_label == "truth_init":
        p0 = p_true.copy()
    elif pos_label == "east_10km":
        p0 = p_true + 10_000.0 * east
    elif pos_label == "east_100km":
        p0 = p_true + 100_000.0 * east
    else:
        raise ValueError(f"unsupported canonical position profile: {pos_label}")
    if vel_label == "zero_velocity":
        v0 = np.zeros(3, dtype=float)
    elif vel_label == "half_truth_velocity":
        v0 = 0.5 * v_true
    else:
        raise ValueError(f"unsupported canonical velocity profile: {vel_label}")
    return {
