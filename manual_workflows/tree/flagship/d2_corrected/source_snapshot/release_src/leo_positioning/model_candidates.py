...
def base_candidate_row(
    ...
    return {
        ...
        "p0_init_error_m": float(np.linalg.norm(np.asarray(p0_init) - np.asarray(obs["p0_true_m"]))),
        "v_init_error_mps": float(np.linalg.norm(np.asarray(v_init) - np.asarray(obs["v_true_mps"]))),
        "true_speed_mps": float(np.linalg.norm(np.asarray(obs["v_true_mps"]))),
        "true_b0_mps": float(obs.get("b0_true_mps", np.nan)),
        "true_bdot_mps2": float(obs.get("bdot_true_mps2", np.nan)),
...
def finalize_candidate(
    ...
    truth = truth_positions(obs)
    final_err, mean_err, max_err = trajectory_errors(estimated_positions, truth)
    vel_err = float(np.linalg.norm(np.asarray(velocity_est) - np.asarray(obs["v_true_mps"])))
    b0_true = float(obs.get("b0_true_mps", np.nan))
    bdot_true = float(obs.get("bdot_true_mps2", np.nan))
    ...
    row.update(
        {
            ...
            "beta0_error_mps": float(beta0 - b0_true) if np.isfinite(beta0) and np.isfinite(b0_true) else np.nan,
            "beta_dot_error_mps2": float(bdot - bdot_true) if np.isfinite(bdot) and np.isfinite(bdot_true) else np.nan,
            "runtime_ms": float(runtime_ms),
